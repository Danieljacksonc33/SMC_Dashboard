@echo off
echo ============================================
echo Restarting Dashboard Server (Strategy Tester)
echo ============================================
echo.
...
REM Set MT4 tester files folder path
set "FILES_FOLDER=%APPDATA%\MetaQuotes\Terminal\7E6C4A6F67D435CAE80890D8C1401332\tester\files"

echo Changing to tester files folder...
cd /d "%FILES_FOLDER%"
...
if exist "LAUNCH_DASHBOARD_TESTER.trigger" (
    echo Auto-launch trigger detected! Dashboard will open automatically...
    echo.
    del "LAUNCH_DASHBOARD_TESTER.trigger" >nul 2>&1
    set AUTO_LAUNCH=1
) else (
    set AUTO_LAUNCH=0
)
